...
Const Mgame$="VikV3d0_"'for the gnet server list, the game title
Const MGameID$="CpS006"' my GUID for this game or something similar...
...
End If
Local W1:TGadget = CreateWindow( "The Viking Game V3.0", 10,10,500,400,,WINDOW_CENTER | WINDOW_TITLEBAR)
...
Local W1Pan1Lab3:TGadget=CreateLabel("Welcome To "+"The Viking Game V3.0",20,2,410,17, W1Pan1,LABEL_CENTER)
		...
		Case EVENT_GADGETACTION
			Select EventSource()				
				...
				Case W1Pan1But2' about
					Tst1="This is The Viking Game using the Hnefatafl Rules."+Chr$(13)
					Tst1=Tst1+"It is built arround BSNet3 a GUI framework using GNet."+Chr$(13)
					Tst1=Tst1+"To play you need two networked computers with internet access,"+Chr$(13)
					Tst1=Tst1+"allthough they could be on the same LAN."+Chr$(13)				
...
Local GError:Byte=0' counter for 5 retries before notifying connection faliure
Local Ggo:Byte=True' true if exit panel W2GP6 is hidden, ie you can move a piece

'game vars
Local GamePhase:Byte=0' 0=not started/connected, 1=your move, 2=opponents move, 3=game you win, 4=game ends opponent wins
...
If Screensize=1' 640 by 480
	W2=CreateWindow( "The Viking Game V3.0 (basic Hnefatafl Rules)", 10,10,640,480,,WINDOW_CENTER | WINDOW_TITLEBAR)
	...
	W2GP6L3=CreateLabel(Tst1,10,135,200,34,W2GP6,LABEL_CENTER)' 2 lines deep	
	W2GP6B5=CreateButton("Yes",30,175,60,31,W2GP6)	
	W2GP6B6=CreateButton("No",130,175,60,31,W2GP6)
...
Else If Screensize=2' 800 by 600
	W2=CreateWindow("The Viking Game V3.0 (basic Hnefatafl Rules)", 10,10,800,600,,WINDOW_CENTER | WINDOW_TITLEBAR)
	...
	Tst1="Your opponent wants to"+Chr$(13)+"start a fresh game ?"
	W2GP6L3=CreateLabel(Tst1,10,160,247,40,W2GP6,LABEL_CENTER)' 2 lines deep	
	W2GP6B5=CreateButton("Yes",40,210,70,35,W2GP6)	
	W2GP6B6=CreateButton("No",160,210,70,35,W2GP6)	
...
Else'1024 by 768
	W2=CreateWindow( "The Viking Game V3.0 (basic Hnefatafl Rules)", 10,10,1024,768,,WINDOW_CENTER | WINDOW_TITLEBAR)
	...
	Tst1="Your opponent wants to"+Chr$(13)+"start a fresh game ?"
	W2GP6L3=CreateLabel(Tst1,10,170,301,44,W2GP6,LABEL_CENTER)' 2 lines deep	
	W2GP6B5=CreateButton("Yes",50,220,80,40,W2GP6)	
	W2GP6B6=CreateButton("No",195,220,80,40,W2GP6)	
...
Repeat' main loop (ConSet=false do set up network) (ConsSet=true do main game loop)
	...
	End If	
	If IsBlack()=0 Then RedrawEverthing(GameSq,Units)'redraw the canvas (BLACK SCREEN)
		Select EventID()
 	 		Case EVENT_WINDOWCLOSE  
				EndIt1()
			...
			Case EVENT_GADGETACTION' button event codes	
				Select EventSource()
					...
					Case W2GP1B1' create new game, you are a server (as attacker)
						...
						End If					
						HideGadget(W2GP1); ShowGadget(W2GP2); Ggo=True
	...
	Else ' conset=true a connection type has been selected		
		...
		If GTick<>0 Then
			...
			If GTick>1000 Then
				...
				GError=GError+1
				GTick=1
				If GError=3 Then' notify and set program as disconnected after 3 retries
		...
		For remoteObj = EachIn objList' Get type of msg
			...
			If AmCon=True Then ' server is connected to a client, client is connected to server
				...
				Rem	
					Print"-----"
					Print "message type = " + GMType
					Print "message count = " + String(GRec)
					Print "message if any = " + GMsg
					Print "-----"
				End Rem	
					Select GMType
						...
						Case "06" 'opponent has requested a New game							
							If GRec<>GRecNum Then 'act on new message sent							
								ExButCon=3
								ExitCon.ExButDisp(ExButCon)
								HideGadget(W2P4); HideGadget(W2P3); ShowGadget(W2GP6); Ggo=False
						...
						Case "07" ' opponent answering yes start a new game
							If GRec<>GRecNum Then 'act on new message sent
								...
								HideGadget(W2GP6); ShowGadget(W2P3)
								ExitCon.Display0(); Ggo=True
							End If
							SetGNetString localObj,0,"10^"+MSec+"^"+String(GRec)+"^"' acknowlagement (code 10)	
							
						Case "08"' opponet answering no for new game
							If GRec<>GRecNum Then 'act on new message sent						
								GrecNum=GRec' set message counter to new message number							
								ExButCon=0
						...
						Case "20"' a message from your opponent
							If GRec<>GRecNum Then 'act on new message sent
							If MesCol2=0 Then
								SetGadgetColor(W2P1L1,190,250,190); MesCol2=1
							Else
								SetGadgetColor(W2P1L1,150,250,150); MesCol2=0								
							End If								
								GrecNum=GRec' set message counter to new message number
						...
						Case "21" 'client ready to start game, set game start vars					
			...
			Else' AmCon=false no conection acknowlaged				
				Select GMType
					...
					Case "02"' message from server to client yes you can join my game
						...
						MoveInfo(GamePhase,W2P3L4,W2P1L2,MesCol1)' set who's move it is information	
						If MesCol1=0 Then MesCol1=1 Else MesCol1=0; Ggo=True
						'SetGadgetText(W2GP6L2,"Game Status :"+Chr$(13)+"Game Underway")																
				End Select			
			End If
		Next	
	 			...
	 			Select EventData()
					Case 1 'left click
						...
						If GameSq.AreExSq()=True Then
							GameSq.ResetExSq(Units)' draws blank squares over previously deleted units shape														
						End If
						'left click	only allow if its your move and GTick=0 ie previous message acknowlaged	
						'and Ggo=true ie exit game panel is hidden
						If GamePhase=1 And GTick=0 And GGo=True Then'your move
							...
							If GameSq.IsHigh(Temp1)=True Then ' a move to the clicked square
								...
								GCopy="22^"+MSec+"^"+String(GSnt)+"^"+Tst1						
								SetGNetString localObj,0,GCopy' send original square ,new square and Tx3
			...
			Case EVENT_GADGETACTION' button event codes	
				Select EventSource()
					...
					Case W2P3B5' exit/end game button during connected phase
						ExitCon.Display0()
						Ggo=False; HideGadget(W2P3); ShowGadget(W2GP6)
					...
					Case W2GP6B3' end game try to start new game
						If GTick=0 Then' allow if last message has been acknowlaged
							DisableGadget(W2GP6B1); DisableGadget(W2GP6B2); DisableGadget(W2GP6B3); DisableGadget(W2GP6B4)
					...
					Case W2GP6B5' yes 1=exit, 2=dissconnect or 3=new game
						Select ExButCon
							...
							Case 3' yes lets play again
								If GTick=0 Then' allow if last message has been acknowlaged							
									ExButCon=0
									ExitCon.Display0()' reset game vars for fresh game
									If AorD=0 Then AorD=1 Else AorD=0									
									MesPan.Display(2)' connected settings for message panel display									
					...
					Case W2GP6B4' cancel exit/end game during connected phase
						HideGadget(W2GP6); ShowGadget(W2P3); Ggo=True
...
Function RedrawEverthing(Sq1:AllSquares,Un1:AllUnits)'redraws canvas in case of black screen
	...
	For T1=0 To 120
		...
		If T2<>-1 Then
			If Un1.GetStatus(T2)=1 Then' the unit exsists (ie not deleted)
				Sq1.DoUnit(Un1.GetOwner(T2),Un1.GetType(T2),T1)
			End If
		End If		
	Next
End Function
...
Function DoInfo(Tf1:TGadget,Tb1:Byte)' Tf1=textfield name
	...
	If Tb1=1 Then ' show help screen	
		...
		Ts1=Ts1+"game using the defending"+S1+"or attacking pieces, or"+S1+"or join a game"+S1+S1		
		AddTextAreaText(Tf1,Ts1)		
		...
		AddTextAreaText(Tf1,Ts1)
		Ts1="THE PLAYER GROUP."+S1+"Is a list of players you"+S1+"can limit a game to."+S1
		Ts1=Ts1+"You can only edit the"+S1+"player group at either the"+S1+"start or end of a game."+S1+S1		
		Ts1=Ts1+"Adding A Player."+S1+"At the end of a game your"+S1+"opponent can be added to"+S1
		...
		AddTextAreaText(Tf1,Ts1)
		Ts1="WHILST PLAYING A GAME."+S1+"When you send a message or"+S1+"make a move the chat panel"+S1		
		Ts1=Ts1+"background will briefly"+S1+"show red. When this is the"+S1+"case you will not be able"+S1
		Ts1=Ts1+"to move or send a message."+S1+S1+"If the colour red persists"+S1+"there may be a connection"+S1					
		Ts1=Ts1+"problem, please be patient"+S1+"as the program will deal"+S1+"with this."+S1+S1	
		Ts1=Ts1+"When the exit/end game"+S1+"panel is showing you will"+S1+"not be able to make a move."+S1+S1		
		AddTextAreaText(Tf1,Ts1)
		Ts1="ENDING THE GAME."+S1+"If you choose to start"+S1+"a new game then you will"+S1
		Ts1=Ts1+"switch sides, ie the"+S1+"attacker becomes the"+S1+"defender."+S1
		AddTextAreaText(Tf1,Ts1)		
...
Type PanCon6' methods for manipulating the exit/end game panel
	...
	Method ExButDisp(Tb1:Byte)'0=hide, 1=end game & exit Y/N, 2=end game & dissconnect, 3=respond to fresh game Y/N
		...
		SetGadgetText(Gad[5],"Game Status :"+Chr$(13)+"Connected")		
		If Tb1<>0 Then
	...
	Method DispWL(Tb1:Byte,Tb2:Byte)' tb1=3 you win, tb1=4 you loose
		'example from viking game alter to suit the curent game
...
Type AllSquares Extends ASquare ' defines all playing squares
	...
	Method ResetExSq(Un1:Allunits)' resets exunit[x] to -1 and draws empty square
		Local Tb1:Byte; Local Tb2%; Local Tb3%
		For Tb1=0 To 5
			Tb2=ExUnit[Tb1]
			If Tb2<>-1 Then
				...
				ExUnit[Tb1]=-1
				Tb3=GetInSq(Tb2)'Tb3=unit number in sq tb2 or -1
				If Tb3<>-1 Then 'Tb3=new unit number in the sq Tb2
					DoUnit(Un1.GetOwner(Tb3),Un1.GetType(Tb3),Tb2)
				End If
			End If
		Next
	End Method
